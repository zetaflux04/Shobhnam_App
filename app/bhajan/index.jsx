...
import { bhajanPackages, bhajanServiceName } from '../../constants/bhajan';
...
export default function BhajanPackages() {
  const router = useRouter();
  const [selectedPackageId, setSelectedPackageId] = useState(bhajanPackages[0]?.id ?? '');
  ...
  const selectedPackage = useMemo(
    () => bhajanPackages.find((pkg) => pkg.id === selectedPackageId) ?? bhajanPackages[0],
    [selectedPackageId],
  );
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={[textVariants.heading3, styles.headerTitle]}>{bhajanServiceName}</Text>
