...
import { bhajanArtists, bhajanReview, bhajanFees } from '../../constants/bhajan';
import { colors, textVariants } from '../../styles/theme';

export default function BhajanArtistProfile() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const artist = bhajanArtists.find((item) => item.id === params.artistId) ?? bhajanArtists[0];

  const gallery = [artist.image, require('../../assets/service/artist 2.png'), require('../../assets/service/artist 3.png')];

  const handleSelectSlot = () => {
    router.push({
      pathname: '/bhajan/booking',
      params: {
        artistId: artist.id,
        packageTitle: 'Customised selection',
        packagePrice: bhajanFees.base.toString(),
      },
    });
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Text style={[textVariants.body3, styles.sectionBody]} numberOfLines={3}>
            Experience devotional Bhajan sandhya performances crafted for your event by skilled artists.
          </Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.reviewCard}>
            <View style={styles.reviewHeader}>
              <Text style={[textVariants.body3, styles.reviewTitle]}>{bhajanReview.name}</Text>
              <Text style={[textVariants.body3, styles.reviewDate]}>{bhajanReview.date}</Text>
              <View style={styles.reviewRating}>
                <Ionicons name="star" size={moderateScale(14)} color="#D29B1B" />
                <Text style={[textVariants.body3, styles.reviewTitle]}>{bhajanReview.rating.toFixed(1)}</Text>
              </View>
            </View>
