...
import { bhajanAddresses, bhajanCalendar, bhajanFees, bhajanTimeSlots } from '../../constants/bhajan';
...
export default function BhajanBooking() {
  ...
  const [selectedAddress, setSelectedAddress] = useState(bhajanAddresses[0]);
  const [selectedDate, setSelectedDate] = useState(bhajanCalendar.highlightedDay ?? 15);
  const [selectedTime, setSelectedTime] = useState(bhajanTimeSlots[0]);

  const packageTitle = params.packageTitle ?? 'Bhajan sandhya';
  const packagePrice = useMemo(() => Number(params.packagePrice ?? bhajanFees.total), [params.packagePrice]);

  const dateLabel = useMemo(
    () => `Sun, ${bhajanCalendar.monthLabel} ${selectedDate.toString().padStart(2, '0')} - ${selectedTime}`,
  ...
  const handleGateChoice = (action) => {
    ...
    if (action === 'sheet') {
      router.push('/bhajan/location-sheet');
    }
  };
  ...
  const handleAddToBag = () => {
    ...
    addToBag({
      ...
      gradient: ['rgba(51,153,117,0.85)', 'rgba(51,153,117,0.4)'],
      image: require('../../assets/service/card-3.png'),
    });
    router.push({
      pathname: '/bhajan/confirmation',
      params: {
        packageTitle,
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.monthRow}>
            ...
            <Text style={[textVariants.button2, styles.monthLabel]}>{bhajanCalendar.monthLabel}</Text>
            <TouchableOpacity activeOpacity={0.85} style={styles.monthArrow}>
