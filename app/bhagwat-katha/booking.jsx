...
import { SafeAreaView, ScrollView, Text, TouchableOpacity, View, Pressable, StyleSheet } from 'react-native';
...
const addresses = [
  {
    id: 'home',
    label: 'Home',
    detail: 'Home - 303B, 3rd floor, Vikas puri 110037,\nNew delhi',
    address: '303B, 3rd floor, Vikas puri',
    city: 'New delhi',
    pinCode: '110037',
  },
  {
    id: 'work',
    label: 'Work',
    detail: 'Work - 619A, 102 floor, New delhi',
    address: '619A, 102 floor',
    city: 'New delhi',
    pinCode: '110001',
  },
];

const timeSlots = ['8:00 AM', '9:00 AM', '6:00 PM', '10:00 PM', '11:00 PM', '12:00 PM'];
...
export default function BhagwatKathaBooking() {
  ...
  const packageTitle = params.packageTitle ?? 'Bahgwat katha';
  const packagePrice = useMemo(() => Number(params.packagePrice ?? 11000), [params.packagePrice]);

  const dateLabel = useMemo(() => `January ${selectedDate}, 2026`, [selectedDate]);
  const eventDate = useMemo(() => new Date(2026, 0, selectedDate, 8, 0, 0).toISOString(), [selectedDate]);

  const handleGateChoice = () => {
    setAddressSaved();
    setHasAddress(true);
  ...
  const handleAddToBag = () => {
    ...
    const dateTime = `Sun, ${dateLabel} - ${selectedTime}`;
    const addr = selectedAddress;
    addToBag({
      artistId: artistId || undefined,
      serviceName: 'Bhagwat Katha',
      packageTitle,
      price: packagePrice,
      dateTime,
      date: eventDate,
      ...
      addressDetail: addr.detail.replace(/\n/g, ' '),
      gradient: ['rgba(69,119,170,0.85)', 'rgba(69,119,170,0.4)'],
      image: require('../../assets/service/card-4.png'),
    });
    router.push({
      pathname: '/bhagwat-katha/confirmation',
      params: {
        packageTitle,
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionHeader}>
          ...
          {hasAddress ? (
            <TouchableOpacity activeOpacity={0.85} onPress={() => setShowAddressGate(true)}>
            ...
            return (
              ...
              >
                ...
                <Text style={[textVariants.body2, styles.addressLabel, isSelected && styles.addressLabelActive]}>
                  {item.label} - {item.detail.split(',')[0].split('-')[1]?.trim() ?? ''}
                </Text>
              </Pressable>
            );
          })}
        ...
        <Text style={[textVariants.body2, styles.addressDetail]}>{selectedAddress.detail}</Text>
        ...
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.monthRow}>
            ...
            <Text style={[textVariants.button2, styles.monthLabel]}>Januari 2026</Text>
            <TouchableOpacity activeOpacity={0.85} style={styles.monthArrow}>
        ...
        <View style={styles.timeRow}>
          {timeSlots.map((slot) => {
            ...
            return (
              ...
              >
                <Text style={[textVariants.body3, styles.timeText, isSelected && styles.timeTextSelected]}>
                  {slot}
                </Text>
              </TouchableOpacity>
            );
      ...
      {showAddressGate && (
        <View style={styles.gateOverlay}>
          <View style={styles.gateSheet}>
            ...
            <TouchableOpacity activeOpacity={0.9} style={styles.gatePrimary} onPress={handleGateChoice}>
