...
import { bhagwatArtists, bhagwatReview } from '../../constants/bhagwat';
import { colors, textVariants } from '../../styles/theme';

export default function BhagwatArtistProfileScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const artist = bhagwatArtists.find((item) => item.id === params.artistId) ?? bhagwatArtists[0];

  const gallery = [artist.image, require('../../assets/service/artist 2.png'), require('../../assets/service/artist 3.png')];

  const handleSelectSlot = () => {
    router.push({ pathname: '/bhagwat-katha/booking', params: { artistId: artist.id } });
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Text style={[textVariants.body3, styles.sectionBody]} numberOfLines={3}>
            On the TalkNdeal platform, you can receive free services such as detailed birth chart analysis, Ashtakoota
            match, numerology, Panchang, daily update and more.
          </Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.reviewCard}>
            <View style={styles.reviewHeader}>
              <Text style={[textVariants.body3, styles.reviewTitle]}>{bhagwatReview.name}</Text>
              <Text style={[textVariants.body3, styles.reviewDate]}>{bhagwatReview.date}</Text>
              <View style={styles.reviewRating}>
                <Ionicons name="star" size={moderateScale(14)} color="#D29B1B" />
                <Text style={[textVariants.body3, styles.reviewTitle]}>{bhagwatReview.rating.toFixed(1)}</Text>
              </View>
            </View>
