...
import { rudraServiceName } from '../../constants/rudra';

const formatCurrency = (value) => `₹${value.toLocaleString('en-IN', { maximumFractionDigits: 0 })}`;

export default function RudraConfirmation() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const packageTitle = params.packageTitle ?? 'Sangeet package.';
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.detailCard}>
          <Text style={[textVariants.heading4, styles.eventTitle]}>{rudraServiceName}</Text>
