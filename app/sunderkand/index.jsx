...
import { sunderPackages, sunderServiceName } from '../../constants/sunderkand';
...
export default function SunderkandPackages() {
  const router = useRouter();
  const [selectedPackageId, setSelectedPackageId] = useState(sunderPackages[0]?.id ?? '');
  ...
  const selectedPackage = useMemo(
    () => sunderPackages.find((pkg) => pkg.id === selectedPackageId) ?? sunderPackages[0],
    [selectedPackageId],
  );
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={[textVariants.heading3, styles.headerTitle]}>{sunderServiceName}</Text>
