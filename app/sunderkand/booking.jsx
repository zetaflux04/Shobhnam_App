...
import { sunderAddresses, sunderCalendar, sunderFees, sunderTimeSlots } from '../../constants/sunderkand';
...
export default function SunderBooking() {
  ...
  const [selectedAddress, setSelectedAddress] = useState(sunderAddresses[0]);
  const [selectedDate, setSelectedDate] = useState(sunderCalendar.highlightedDay ?? 15);
  const [selectedTime, setSelectedTime] = useState(sunderTimeSlots[0]);

  const packageTitle = params.packageTitle ?? 'Sunderkand';
  const packagePrice = useMemo(() => Number(params.packagePrice ?? sunderFees.total), [params.packagePrice]);

  const dateLabel = useMemo(
    () => `Sun, ${sunderCalendar.monthLabel} ${selectedDate.toString().padStart(2, '0')} - ${selectedTime}`,
  ...
  const handleGateChoice = (action) => {
    ...
    if (action === 'sheet') {
      router.push('/sunderkand/location-sheet');
    }
  };
  ...
  const handleAddToBag = () => {
    ...
    addToBag({
      ...
      gradient: ['rgba(237,122,39,0.85)', 'rgba(237,122,39,0.4)'],
      image: require('../../assets/service/card-2.png'),
    });
    router.push({
      pathname: '/sunderkand/confirmation',
      params: {
        packageTitle,
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.monthRow}>
            ...
            <Text style={[textVariants.button2, styles.monthLabel]}>{sunderCalendar.monthLabel}</Text>
            <TouchableOpacity activeOpacity={0.85} style={styles.monthArrow}>
