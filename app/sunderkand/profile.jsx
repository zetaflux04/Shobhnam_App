...
import { sunderArtists, sunderReview, sunderFees } from '../../constants/sunderkand';
import { colors, textVariants } from '../../styles/theme';

export default function SunderArtistProfile() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const artist = sunderArtists.find((item) => item.id === params.artistId) ?? sunderArtists[0];

  const gallery = [artist.image, require('../../assets/service/artist 2.png'), require('../../assets/service/artist 3.png')];

  const handleSelectSlot = () => {
    router.push({
      pathname: '/sunderkand/booking',
      params: {
        artistId: artist.id,
        packageTitle: 'Customised selection',
        packagePrice: sunderFees.base.toString(),
      },
    });
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Text style={[textVariants.body3, styles.sectionBody]} numberOfLines={3}>
            Experience soulful recitations and authentic Sunderkand performances tailored to your event with seasoned artists.
          </Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.reviewCard}>
            <View style={styles.reviewHeader}>
              <Text style={[textVariants.body3, styles.reviewTitle]}>{sunderReview.name}</Text>
              <Text style={[textVariants.body3, styles.reviewDate]}>{sunderReview.date}</Text>
              <View style={styles.reviewRating}>
                <Ionicons name="star" size={moderateScale(14)} color="#D29B1B" />
                <Text style={[textVariants.body3, styles.reviewTitle]}>{sunderReview.rating.toFixed(1)}</Text>
              </View>
            </View>
