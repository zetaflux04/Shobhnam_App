...
export default function OtherServicesConfirmation() {
  ...
  const addressLine =
    params.addressLine ?? 'Home - 303B, 3rd floor, Vikaspuri 110037, New delhi';
  const dateLabel = params.dateLabel ?? 'Sun, January 15, 2026 - 08:00 AM';

  const displayServiceTitle = serviceTitle.replace(/\.$/, '');
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Text style={[textVariants.body2, styles.statusSubtitle]}>
          Your order has been added to the bag. Go to bag to review and confirm payment.
        </Text>

        <View style={styles.detailCard}>
          <Text style={[textVariants.heading4, styles.eventTitle]}>{displayServiceTitle}</Text>
          <Text style={[textVariants.body3, styles.eventSubtitle]}>{artistName}</Text>
          ...
          <View style={styles.row}>
            <View style={styles.datePill}>
              <Text style={[textVariants.body3, styles.datePillText]}>{dateLabel}</Text>
            </View>
          </View>
