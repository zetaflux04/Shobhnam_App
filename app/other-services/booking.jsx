...
import {
  ...
  otherServiceCalendar,
  otherServiceTimeSlots,
} from '../../constants/otherServices';
...
export default function OtherServicesBooking() {
  ...
  const [selectedAddress, setSelectedAddress] = useState(otherServiceAddresses[0]);
  const [selectedDate, setSelectedDate] = useState(otherServiceCalendar.highlightedDay ?? 15);
  const [selectedTime, setSelectedTime] = useState(otherServiceTimeSlots[0]);

  const serviceTitle = params.serviceTitle ?? 'Light, sound and tent crew.';
  const artistName = params.artistName ?? 'Suresh and camera crew';
  const packagePrice = useMemo(
    () => Number(params.packagePrice ?? 11000),
    [params.packagePrice],
  );

  const dateLabel = useMemo(
    () => `Sun, ${otherServiceCalendar.monthLabel} ${selectedDate.toString().padStart(2, '0')} - ${selectedTime}`,
  ...
  const handleAddToBag = () => {
    ...
    const packageTitleDisplay = `${serviceTitle} - ${artistName}`;
    addToBag({
      serviceName: 'Other services',
      packageTitle: packageTitleDisplay,
      price: packagePrice,
      dateTime: dateLabel,
      date: eventDate,
      address: addr.full,
      ...
      gradient: ['rgba(165,52,102,0.85)', 'rgba(165,52,102,0.4)'],
      image: require('../../assets/service/card-6.png'),
    });
    router.push({
      pathname: '/other-services/confirmation',
      params: {
        serviceTitle,
        artistName,
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.addressRow}>
          {otherServiceAddresses.map((item) => {
        ...
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.monthRow}>
            ...
            <Text style={[textVariants.button2, styles.monthLabel]}>{otherServiceCalendar.monthLabel}</Text>
            <TouchableOpacity activeOpacity={0.85} style={styles.monthArrow}>
        ...
        <View style={styles.timeRow}>
          {otherServiceTimeSlots.map((slot) => {
