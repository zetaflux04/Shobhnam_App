...
export default function OtherServicesArtistProfile() {
  const router = useRouter();
  const { artistId, serviceId } = useLocalSearchParams();
  ...
  const handleSelectSlot = () => {
    router.push({
      pathname: '/other-services/booking',
      params: {
        artistId: artist.id,
        serviceId,
        artistName: artist.name,
        serviceTitle,
        packagePrice: artist.price.toString(),
      },
    });
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <Text style={[textVariants.body3, styles.sectionBody]} numberOfLines={3}>
            On the Talkndheal platform, you can receive free services such as detailed birth chart analysis, Ashtakoota
        ...
        <View style={styles.section}>
          ...
          <View style={styles.reviewCard}>
            <View style={styles.reviewHeader}>
              <Text style={[textVariants.body3, styles.reviewTitle]}>{otherServiceReview.name}</Text>
              <Text style={[textVariants.body3, styles.reviewDate]}>{otherServiceReview.date}</Text>
              <View style={styles.reviewRating}>
                <Ionicons name="star" size={moderateScale(14)} color="#D29B1B" />
                <Text style={[textVariants.body3, styles.reviewTitle]}>{otherServiceReview.rating.toFixed(1)}</Text>
              </View>
            </View>
            <Text style={[textVariants.body3, styles.reviewBody]} numberOfLines={3}>
              {otherServiceReview.text}
            </Text>
            <Text style={[textVariants.body3, styles.readMore]}>Read more</Text>
          </View>
