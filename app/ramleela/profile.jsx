...
import { ramleelaArtists, ramleelaReview, ramleelaFees } from '../../constants/ramleela';
import { colors, textVariants } from '../../styles/theme';

export default function RamleelaArtistProfileScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const artist = ramleelaArtists.find((item) => item.id === params.artistId) ?? ramleelaArtists[0];
  ...
  const handleSelectSlot = () => {
    router.push({
      ...
      params: {
        ...
        packagePrice: ramleelaFees.base.toString(),
      },
    });
  };
        ...
        <View style={styles.section}>
          ...
          <View style={styles.reviewCard}>
            <View style={styles.reviewHeader}>
              <Text style={[textVariants.body3, styles.reviewTitle]}>{ramleelaReview.name}</Text>
              <Text style={[textVariants.body3, styles.reviewDate]}>{ramleelaReview.date}</Text>
              <View style={styles.reviewRating}>
                <Ionicons name="star" size={moderateScale(14)} color="#D29B1B" />
                <Text style={[textVariants.body3, styles.reviewTitle]}>{ramleelaReview.rating.toFixed(1)}</Text>
              </View>
            </View>
