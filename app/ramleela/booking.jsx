...
import { ramleelaAddresses, ramleelaCalendar, ramleelaFees, ramleelaTimeSlots } from '../../constants/ramleela';
...
export default function RamleelaBooking() {
  ...
  const [selectedAddress, setSelectedAddress] = useState(ramleelaAddresses[0]);
  const [selectedDate, setSelectedDate] = useState(ramleelaCalendar.highlightedDay ?? 15);
  const [selectedTime, setSelectedTime] = useState(ramleelaTimeSlots[0]);

  const packageTitle = params.packageTitle ?? 'Ramleela';
  const packagePrice = useMemo(() => Number(params.packagePrice ?? ramleelaFees.total), [params.packagePrice]);

  const dateLabel = useMemo(
    () => `Sun, ${ramleelaCalendar.monthLabel} ${selectedDate.toString().padStart(2, '0')} - ${selectedTime}`,
  ...
  const handleAddToBag = () => {
    ...
    addToBag({
      ...
      gradient: ['rgba(255,167,38,0.85)', 'rgba(255,167,38,0.4)'],
      image: require('../../assets/service/card-1.png'),
    });
    router.push({
      pathname: '/ramleela/confirmation',
      params: {
        packageTitle,
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.monthRow}>
            ...
            <Text style={[textVariants.button2, styles.monthLabel]}>{ramleelaCalendar.monthLabel}</Text>
            <TouchableOpacity activeOpacity={0.85} style={styles.monthArrow}>
